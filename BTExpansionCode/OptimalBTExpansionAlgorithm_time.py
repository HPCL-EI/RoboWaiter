...
class OptBTExpAlgorithm:
    ...
    def run_algorithm_selTree(self, start, goal, actions,merge_time=3):
        # self.scene = scene
        ...
        while len(self.nodes)!=0:
            ...
            end_time=time.time()
            time_dic["choose_min_c"] +=end_time-start_time
            # ======================== End Next Goal ============================ #
            ...
            end_time = time.time()
            time_dic["act_traverse_time"] = (end_time - start_time)
            # ====================== End Action Trasvers ============================ #
    ...
    def run_algorithm(self, start, goal, actions,merge_time=3):
        ...
        if len(goal) > 1:
            for g in goal:
                bt_sel_tree,mincost,time_dic = self.run_algorithm_selTree(start, g, actions)
            ...
            self.bt,mincost,time_dic = self.run_algorithm_selTree(start, goal[0], actions,merge_time=merge_time)
        return True

    def run_algorithm_test(self, start, goal, actions):
        self.bt,mincost,time_dic = self.run_algorithm_selTree(start, goal, actions)
        return True,time_dic


    def merge_adjacent_conditions_stack_time(self,bt_sel,merge_time=3):
